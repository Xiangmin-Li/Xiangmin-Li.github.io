...
{
    public abstract void absHelloWorld();//抽象方法必须被重写
    public void defaultHelloWorld()//默认方法
    {
        Console.WriteLine("baseClass-default Hello World");
    }
    public virtual void virHelloWorld()//允许被重写的方法
    {
        Console.WriteLine("baseClass-vir Hello World");
    }
    public virtual void noOverrideVirHelloWorld() {
        Console.WriteLine("baseClass-no-override-vir-Hello World");
    }
}

class sanClass : baseClass
{
    public override void absHelloWorld()//重写父类absHelloWorld方法
    {
        Console.WriteLine("sanClass-abs-Hello World");
    }
    public override void virHelloWorld()//重写父类virHelloWorld方法 
    {
        base.virHelloWorld();//调用父类方法
        Console.WriteLine("sanClass-vir-Hello World");
    }
}
...
internal class Program
{
    static void Main(string[] args)
    {
        ...
        sanClass.absHelloWorld();//输出"sanClass-abs-Hello World"
        sanClass.defaultHelloWorld();//输出"baseClass-default Hello World"
        sanClass.virHelloWorld();//输出"baseClass-vir Hello World" "sanClass-vir-Hello World"
        sanClass.noOverrideVirHelloWorld();//输出"baseClass-no-override-vir-Hello World"
        Console.WriteLine(sanClass.GetType());//输出sanClass类型

        baseClass baseClass = new sanClass();
        baseClass.absHelloWorld();//输出"sanClass-abs-Hello World"
        baseClass.defaultHelloWorld();//输出"baseClass-default Hello World"
        baseClass.virHelloWorld();//输出"baseClass-vir Hello World" "sanClass-vir-Hello World"
        baseClass.noOverrideVirHelloWorld();//输出"baseClass-no-override-vir-Hello World"
        Console.WriteLine(baseClass.GetType());//输出sanClass类型

        //baseClass bs = new baseClass();无法创建抽象类的实例
    }
}
...
//接口的实现1
class LocalSave : ISaveSystem
{
    public void Save()
    {
        Console.WriteLine("存档已保存到本地");
    }
    public string Read()
    {
        Console.WriteLine("本地读取存档");
        return "本地存档数据";
    }
}
//接口的实现2
class CloudSave : ISaveSystem
{
    public void Save()
    {
        Console.WriteLine("存档已保存到云");
    }
    public string Read()
    {
        Console.WriteLine("读取云端存档");
        return "云端存档数据";
    }
}
//高层模块-依赖抽象而不是具体实现
class Game { 
    ...
    public Game(ISaveSystem _saveSystem)
    {
        saveSystem = _saveSystem;
    }

    public void toSaveMyGame() {
        Console.WriteLine("保存游戏");
        saveSystem.Save();
    }
    public string myGameData { get; set; }
    public void toReadMyGame() {
        Console.WriteLine("读取游戏");
        myGameData= saveSystem.Read();
    }
}
...
internal class Program
{
    static void Main(string[] args)
    {
        ISaveSystem saveSystem = new CloudSave();
        Game game = new Game(saveSystem);

        game.toSaveMyGame();
        game.toReadMyGame();
        Console.WriteLine($"读取到的存档数据为{game.myGameData}");
    }
}
```
